  ...
  return (
    <div style={{minHeight:'100%',padding:'1.5rem 1.75rem',maxWidth:720,margin:'0 auto'}}>
      <div style={{display:'flex',alignItems:'center',gap:'0.75rem',marginBottom:'1.5rem'}}>
        <button onClick={()=>navigate('/jobs')} style={{width:34,height:34,borderRadius:'0.5rem',border:'1px solid var(--gb)',background:'var(--g2)',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',color:'var(--tx3)'}}>
          <ArrowLeft size={15}/>
        </button>
        <div>
          <h1 style={{color:'var(--tx1)',fontSize:'1.2rem',fontWeight:800,letterSpacing:'-0.02em',margin:0}}>Create Job</h1>
          <p style={{color:'var(--tx4)',fontSize:'0.78rem',margin:0}}>Plan a job from an active DO and assign an agent</p>
        </div>
      </div>
        ...
        <div style={card}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.75rem'}}>
            <div style={{gridColumn:'1/-1'}}>
              <label style={lbl}>Delivery Order *</label>
              <select style={inp} value={form.do_id} onChange={e=>setField('do_id',e.target.value)}>
                <option value="">Select active DO…</option>
                {activeDOs.map(d=><option key={d.id} value={d.id}>{d.do_number} — {d.supplier?.name}</option>)}
              </select>
            </div>
            <div>
            ...
            <div style={{gridColumn:'1/-1'}}>
              ...
              <textarea style={{...inp,resize:'vertical'} as React.CSSProperties} rows={3} value={form.processing_instructions} onChange={e=>setField('processing_instructions',e.target.value)} placeholder="Cut sizes, tolerances, packing requirements…"/>
            </div>
          </div>
        </div>

        <div style={{display:'flex',gap:'0.75rem',paddingBottom:'2rem'}}>
          <button type="button" onClick={()=>navigate('/jobs')} style={{flex:1,padding:'0.7rem',borderRadius:'0.6rem',border:'1px solid var(--gb)',background:'var(--g2)',color:'var(--tx2)',fontWeight:600,fontSize:'0.88rem',cursor:'pointer'}}>Cancel</button>
          <button type="submit" disabled={submitting} style={{flex:1,padding:'0.7rem',borderRadius:'0.6rem',border:'none',background:'linear-gradient(135deg,#a78bfa,#7c3aed)',color:'#fff',fontWeight:700,fontSize:'0.88rem',cursor:'pointer',opacity:submitting?0.65:1}}>
